...
public class TextRadar extends HUDModule {
	...
	public void onRender() {
		...
		mc.world.loadedEntityList.stream()
			...
			.forEach(e -> {
				...
				if (display.getValue().equalsIgnoreCase("Friend") && !(SocialManager
					.isFriend(e.getName()))) {
					return;
				}
				if (display.getValue().equalsIgnoreCase("Enemy") && !(SocialManager
					.isEnemy(e.getName()))) {
					return;
				}
	...
	private class PlayerList implements HUDList {
		...
		public String getItem(int index) {
			...
			return TextFormatting.GRAY + "[" + healthcolor + ((int) health) + TextFormatting.GRAY
				+ "] " + friendcolor + e.getName() + TextFormatting.GRAY + " [" + distancecolor
				+ ((int) distance) + TextFormatting.GRAY + "]";
		}
