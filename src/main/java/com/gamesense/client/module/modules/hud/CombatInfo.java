...
public class CombatInfo extends HUDModule {
	...
	private static final BlockPos[] surroundOffset = new BlockPos[]{new BlockPos(0, 0, -1),
		new BlockPos(1, 0, 0), new BlockPos(0, 0, 1), new BlockPos(-1, 0, 0)};
	private static final String[] hoosiersModules = {"AutoCrystalGS", "KillAura", "Surround",
		"AutoTrap", "SelfTrap"};
	...
	public void onRender() {
		...
		list.totems = mc.player.inventory.mainInventory.stream()
			...
			(mc.player.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING) ? 1 : 0);
		...
		List<EntityPlayer> entities = new ArrayList<EntityPlayer>(mc.world.playerEntities.stream()
		...
		for (EntityPlayer e : entities) {
			...
			for (BlockPos add : surroundOffset) {
				++i;
				BlockPos o = new BlockPos(e.getPositionVector().x, e.getPositionVector().y,
					e.getPositionVector().z).add(add.getX(), add.getY(), add.getZ());
				if (mc.world.getBlockState(o).getBlock() == Blocks.OBSIDIAN) {
					if (i == 1 && CrystalUtil.canPlaceCrystal(o.north(1).down(),
						autoCrystalGS.endCrystalMode.getValue())) {
						list.lby = true;
						list.renderLby = true;
					} else if (i == 2 && CrystalUtil.canPlaceCrystal(o.east(1).down(),
						autoCrystalGS.endCrystalMode.getValue())) {
						list.lby = true;
						list.renderLby = true;
					} else if (i == 3 && CrystalUtil.canPlaceCrystal(o.south(1).down(),
						autoCrystalGS.endCrystalMode.getValue())) {
						list.lby = true;
						list.renderLby = true;
					} else if (i == 4 && CrystalUtil.canPlaceCrystal(o.west(1).down(),
						autoCrystalGS.endCrystalMode.getValue())) {
						list.lby = true;
						list.renderLby = true;
					}
