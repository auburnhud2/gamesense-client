...
public class MessageBus {
	...
	public static String watermark =
		ChatFormatting.GRAY + "[" + ChatFormatting.WHITE + "Game" + ChatFormatting.GREEN + "Sense"
	...
	public static void sendClientPrefixMessage(String message) {
		TextComponentString string1 = new TextComponentString(
			watermark + messageFormatting + message);
	...
	/**
	 * Command-oriented message, with the nature of commands we don't want them being a
	 * notification
	 **/
	public static void sendCommandMessage(String message, boolean prefix) {
		...
		TextComponentString string = new TextComponentString(
			watermark1 + messageFormatting + message);
	...
	public static void sendClientRawMessage(String message) {
		...
		if (ModuleManager.isModuleEnabled(Notifications.class) && notifications.disableChat
			.getValue()) {
			return;
		}
		mc.player.sendMessage(string);
