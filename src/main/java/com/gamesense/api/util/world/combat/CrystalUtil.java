...
public class CrystalUtil {
	...
	public static boolean canPlaceCrystal(BlockPos blockPos, boolean newPlacement) {
		if (!isValidBlock(mc.world.getBlockState(blockPos).getBlock())) {
			return false;
		}
		...
		if (newPlacement) {
			...
			if (!isValidMaterial(mc.world.getBlockState(posUp).getMaterial())
				|| !isValidMaterial(mc.world.getBlockState(posUp.up()).getMaterial())) {
				return false;
			}
		}
	...
	public static List<BlockPos> findCrystalBlocks(float placeRange, boolean newPlacement) {
		...
		positions.addAll(EntityUtil
			.getSphere(PlayerUtil.getPlayerPos(), placeRange, (int) placeRange, false, true, 0)
			.stream().filter(pos -> CrystalUtil.canPlaceCrystal(pos, newPlacement))
